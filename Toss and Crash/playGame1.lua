...
function scene:enterScene(event)	
		...
		sky.x = display.contentWidth; sky.y = (display.contentHeight-display.contentHeight /2)
		game:insert( sky )
		
		--sky.x = display.contentWidth-display.contentWidth /2; sky.y = display.contentHeight+display.contentHeight /2
		...
		game:insert( player )
		physics.addBody( player, { density=10.0, friction=10.0, bounce=0.0, radius=25 } )
		player.bodyType = "kinematic"
		player.x = 50
		player.y = 290

		local player2 	= display.newImage( "graphics/redplayer.png" )
		local easingx  	= require("easing");
		game:insert( player2 )
		physics.addBody( player2, { density=10.0, friction=10.0, bounce=0.0, radius=25 } )
		...
		local function resetplayer()
			if playerCount == 0 then
				...
				player:setLinearVelocity( 0, 0 ) -- stop player moving
				player.angularVelocity = 0 -- stop player rotating				
			elseif playerCount == 1 then
				destroyObj(player)
				player2.bodyType = "kinematic"
				transition.to(player2, {time=600, y=200, transition = easingx.easeOutElastic})
				player2:setLinearVelocity( 0, 0 ) -- stop player moving
				player2.angularVelocity = 0 -- stop player rotating
			end	
		...
		local function checkGame()
			...
			elseif deadPig < 3 and playerRemain == 0 then
		...
		local function dropplayer ( event )
			...
			elseif event.phase == "ended" then
				...
				elseif playerCount == 1 then
					...
					player2:applyForce((50 - event.x)*200, (200 - event.y)*200, player2.x, player2.y)
					playerRemain = playerRemain - 1
				end	
				startListening()
